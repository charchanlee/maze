...
int dfs(int row, int col, int goal_row, int goal_col, int num_rows,
    ...
    if (r-> row == goal_row && r-> column == goal_col){
        r->next = 0;
        return 1;
    }
    r->visited = 1; 
    for (int i = 0; i < 4; i++){
        ...
        struct maze_room *neighbor = get_neighbor(num_rows, num_cols, maze, r, dir);
        if (neighbor != 0 && r->walls[dir] == 0 && neighbor->visited == 0) {
            #ifdef FULL 
            int err = fprintf(file, "%i, %i\n", neighbor->row, neighbor->column);
            if (err<0) 
                return 1; 
            #endif 
            if (dfs(neighbor->row, neighbor->column, goal_row, goal_col, num_rows, num_cols, maze, file)) {
                r->next = neighbor;
                return 1;
            }
            else {
...
void decode_maze(int num_rows, int num_cols,
    ...
    for (int i = 0; i < num_rows; i++) {
        for (int j = 0; j < num_cols; j++) {
            struct maze_room *r = &maze[i][j];
            create_room_connections(r, encoded_maze[i][j]);
        }
    }
}
...
int main(int argc, char **argv) {
    ...
    int read_err = read_encoded_maze_from_file(num_rows, num_cols, encoded_maze, maze_file_name);
    if (read_err == 1) {
        return 1;
    }
    ...
    fprintf(file, "PRUNED\n");
    dfs(start_row, start_col, goal_row, goal_col, num_rows, num_cols, maze, file);
    print_pruned_path(&maze[start_row][start_col], file);
    #endif
    return 0; 
}
